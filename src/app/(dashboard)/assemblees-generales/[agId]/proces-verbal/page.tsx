...
export default function ProcesVerbalPage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function loadOpposantsDefaillants() {
      if (!selectedCopro || !resolutions.length) {
  ...
  }, [selectedCopro, agId, resolutions]);

  if (coproLoading || agLoading || resolutionsLoading || presencesLoading || loadingOpposants) {
  ...
  const handleFinalize = async () => {
    if (!user || ag.statut !== 'en_cours') return;
