...
export default function FinancesPage() {
  ...
  useEffect(() => {
    ...
    const unsubAppels = subscribeToAppels(condoId, (updated) => {
      setAppels(updated);
      setLoading(false);
    });

    const unsubPaiements = subscribeToPaiements(condoId, (updated) => {
      setPaiements(updated);
    });

    const unsubCopros = subscribeToCoproprietaires(condoId, (updated) => {
  ...
  const handleDeleteAppel = async () => {
    if (!deleteAppelTarget || !currentCondo || !user) return;

    setIsDeleting(true);
    try {
      await deleteAppel(currentCondo.id, deleteAppelTarget.id, user.uid, user.email || '');
  ...
  const handleDeletePaiement = async () => {
    if (!deletePaiementTarget || !currentCondo || !user) return;

    setIsDeleting(true);
    try {
      await deletePaiement(currentCondo.id, deletePaiementTarget.id, user.uid, user.email || '');
      ...
      <div role="tabpanel">
          ...
          <>
            {canWrite && (
              <div className="mb-4 flex justify-end">
                <Link href={condoPaths.appelNew(condoId)}>
                  <Button>
        ...
        {activeTab === 'paiements' && (
          <>
            {canWrite && (
              <div className="mb-4 flex justify-end">
                <Link href={condoPaths.paiementNew(condoId)}>
                  <Button>
                    <Plus className="mr-2 h-4 w-4" />
