...
export default function ConvocationPage({ params }: PageProps) {
  ...
  if (condoLoading || agLoading || resolutionsLoading) {
  ...
  const handleMarkAsSent = async () => {
    if (!user || ag.statut !== 'brouillon' || !condoId) return;

    setIsTransitioning(true);
    try {
      await transitionAGStatus(condoId, agId, user.uid, user.email || '', 'convoquee');
    } catch (err) {
