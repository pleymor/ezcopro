...
export default function SoldesPage() {
  ...
  useEffect(() => {
    ...
    const unsubCopros = subscribeToCoproprietaires(selectedCopro.id, setCoproprietaires);
  ...
  // Calculer les soldes quand les données changent
  useEffect(() => {
    if (!selectedCopro || coproprietaires.length === 0) {
      setLoading(false);
      return;
    }
    ...
    loadSoldes();
  }, [selectedCopro, coproprietaires, lots, paiements]);
                  ...
                  {filteredSoldes.map((solde) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <Link
                          href={`/soldes/${solde.coproprietaireId}`}
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/soldes/${solde.coproprietaireId}`}>
