    ...
    return (
      <div className="rounded-lg border border-dashed p-8 text-center">
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-center">
          ...
          <Link href={'/lots/cles-repartition/nouvelle' as Route}>
            <Button>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {cles.map((cle) => (
          <CleRepartitionCard key={cle.id} cle={cle} />
        ))}
      </div>
    </div>
  );
...
function CleRepartitionCard({ cle }: CleRepartitionCardProps) {
  ...
  return (
    <Link href={`/lots/cles-repartition/${cle.id}` as Route}>
      <div className="group rounded-lg border bg-card p-4 transition-colors hover:bg-muted/50">
        ...
        <div className="mt-4 flex items-center gap-4 text-sm">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className={isValid ? 'font-medium text-green-600' : 'font-medium text-amber-600'}>
              {percentage}%
            </span>
            {!isValid && (
